...
function twentig_twentyone_customize_controls_enqueue_scripts() {
	...
	wp_localize_script(
		...
		array(
			...
			'editorUrl'    => esc_url( admin_url( 'post-new.php?post_type=page&twentig_tour_editor=1' ) ),
		)
	);
	wp_enqueue_script(
		'twentig-starters',
		TWENTIG_ASSETS_URI . '/js/twentig-starters.js',
		array(),
		TWENTIG_VERSION,
		true
	);
	...
	// Loads editor scripts for our Welcome Guide if Widgets Block Editor is disabled.
	if ( ! function_exists( 'wp_use_widgets_block_editor' ) || ( function_exists( 'wp_use_widgets_block_editor' ) && ! wp_use_widgets_block_editor() ) ) {
